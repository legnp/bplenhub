  ...
  return (
    <div className={`w-full ${step === 'calendar' ? 'max-w-4xl' : 'max-w-xl'} mx-auto p-4 sm:p-8 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[40px] shadow-2xl relative overflow-hidden theme-dark group transition-all duration-700`}>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "triagem" && (
          <motion.div key="triagem" {...containerVariants} className="space-y-6 relative z-10">
             <div className="flex items-center gap-2">
              <button onClick={() => setStep("lead")} className="p-2 hover:bg-white/5 rounded-full transition-all">
                <ChevronLeft className="w-5 h-5 text-white/40" />
              </button>
              <h3 className="text-xl font-black text-white">Triagem BPlen</h3>
            </div>
        ...
        {step === "calendar" && (
          <motion.div key="calendar" {...containerVariants} className="space-y-8 relative z-10">
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-[1fr_320px] gap-8">
              ...
              <div className="space-y-6">
                      ...
                      <React.Fragment key={day.toString()}>
                        ...
                        >
                          ...
                          {isSameDay(day, new Date()) && !isSelected && (
                            <div className="absolute bottom-1.5 w-1 h-1 rounded-full bg-[#667eea]" />
                          )}
                        </button>
                      </React.Fragment>
                    );
                ...
                <div className="flex items-start gap-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                  <div className="shrink-0 w-6 h-6 flex items-center justify-center bg-[#764ba2]/10 rounded border border-[#764ba2]/20">
                    <span className="text-[9px] font-black text-[#764ba2]">SI</span>
                  </div>
                  <div>
                    <h5 className="text-[9px] font-black text-white/50 uppercase tracking-widest leading-none mb-1">Semana ISO</h5>
                    <p className="text-[9px] text-white/20 font-medium leading-relaxed italic">
                      Padrão internacional de contagem para sincronização estratégica (SI-01 a SI-52).
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  {isLoadingSlots ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center py-20">
                      ...
                      <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest leading-relaxed">
                        Nenhum horário livre <br /> para hoje.
                      </p>
                    </div>
                  ) : (
